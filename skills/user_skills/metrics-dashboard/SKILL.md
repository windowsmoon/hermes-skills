...
description: >
  当需要设计产品指标仪表盘时使用。
  用于定义北极星指标、输入指标、健康指标、业务指标，设计数据源、可视化类型和告警阈值。
  不要用于定义北极星指标本身（用north-star-metric）、A/B测试分析（用ab-test-analysis）或用户行为数据分析（用cohort-analysis）。
  触发词：指标仪表盘、KPI仪表盘、产品指标、数据看板、指标设计
