...
description: >
  当需要进行全面的竞品分析、识别5个直接竞品并绘制竞争格局时使用。
  用于竞争情报收集、竞品优劣势分析、差异化机会发现、竞争态势评估。
  不要用于针对单一竞品的销售对战卡（用competitive-battlecard）、行业宏观分析（用PESTLE）或单个功能点的功能对比。
  触发词：竞品分析、竞争分析、竞争格局、竞争对手分析、市场竞品
