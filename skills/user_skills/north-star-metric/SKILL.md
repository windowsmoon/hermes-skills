...
description: >
  当需要定义北极星指标（North Star Metric）时使用。
  用于识别业务游戏类型（注意力/交易/生产力），定义北极星指标和3-5个输入指标，按7条标准验证。
  不要用于设计完整指标仪表盘（用metrics-dashboard）、OKR制定（用brainstorm-okrs）或AARRR漏斗分析。
  触发词：北极星指标、North Star、关键指标、OMTM、核心指标
