...
description: >
  当需要构建机会解决方案树（OST）来结构化产品发现工作时使用。
  用于将期望结果映射到客户机会、解决方案和实验，基于Teresa Torres的持续发现习惯。
  不要用于PRD编写（用create-prd）、功能优先级排序（用prioritize-features）或用户故事编写（用user-stories）。
  触发词：机会解决方案树、OST、产品发现、持续发现、机会映射
