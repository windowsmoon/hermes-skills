...
- [references/commands.md](references/commands.md) — 所有命令及参数（执行方式：`sh dist/run.sh <command>` 或 `node dist/paotui.js <command>`）
...
- [references/install-and-auth.md](references/install-and-auth.md) — 安装与授权记录（Windows 环境下用 `node dist/paotui.js` 而非 `sh dist/run.sh`）
