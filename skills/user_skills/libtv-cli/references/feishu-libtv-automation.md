...
```bash
libtv node create "<节点名>-图" -t image \
  --prompt "<提示词>" \
  -s "model=<图片_模型>" \
  -s count=<图片_生成数量> \
  -s quality=<图片_分辨率> \
...
```bash
libtv node create "镜头01-图" -t image \
  --prompt "赛博朋克风格城市夜景" \
  -s "model=Lib Navo 2" \
  -s count=1 \
  -s quality=2K \
...
```bash
libtv node create "<节点名>-视频" -t video \
  --prompt "<分镜描述或提示词>" \
  -s "model=<视频_模型>" \
...
### 典型出视频命令（Seedance 2.0 VIP）

```bash
libtv node create "镜头01-视频" -t video \
  --prompt "无人机掠过海面，夕阳下的城市天际线" \
  -s "model=Seedance 2.0 VIP" \
  -s modeType=mixed2video \
  -s count=1 \
  -s ratio=16:9 \
  -s resolution=720p \
...
1. 读取飞书多维表格每行记录（通过 `lark-cli base +record-list` 或 tenant_access_token API）
2. 对 `状态=待处理` 的行：
   - 调用 `feishu-cli/scripts/get_bitable_urls.py` 或直接调 `batch_get_tmp_download_url` API 获取图片/音频/视频的 `tmp_download_url`
   - 直接将 `tmp_url` 作为参数传给 `libtv node create --left <url>` **（无需下载到本地）**
   - 若工具不支持远程 URL，则先用 `lark-cli +record-download-attachment` 下载到 `D:\hermes-data\attachments\`，再传本地路径
   - 用 `分镜描述` 作为视频 prompt，`提示词` 作为图片 prompt
   - 将生成的图片/视频 URL 回填到飞书多维表格的 `图片`、`视频`、`视频URL` 字段
   - 更新 `状态` 为 `完成`
...
| 方式 | 优点 | 缺点 |
|------|------|------|
| `get_bitable_urls.py` 脚本 | 一键输出 JSON，AI 直接使用 | 需要传入 app_id/app_secret |
| `lark-cli +record-download-attachment` | 简单直接 | 只能下载到本地，无法直接拿到 URL |

**推荐流程**：
- AI 根据业务意图判断需要哪些 URL → 调用脚本获取 JSON → 将 `_all_urls` 或分类数组传给 libtv 等工具
