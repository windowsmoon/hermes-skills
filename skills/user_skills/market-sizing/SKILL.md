...
name: market-sizing
description: >
  当需要估算市场规模（TAM/SAM/SOM）时使用。
  用于通过自上而下和自下而上两种方法交叉验证市场机会，为投资者路演或市场进入决策提供数据。
  不要用于市场细分（用market-segments）、竞品分析（用competitor-analysis）或财务预测建模。
  触发词：市场规模、TAM、SAM、SOM、市场容量、可服务市场
