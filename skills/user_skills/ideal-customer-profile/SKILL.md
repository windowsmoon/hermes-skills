...
description: >
  当需要从研究数据中识别理想客户画像（ICP）时使用。
  用于分析人口统计、行为模式、JTBD、需求痛点，定义最可能留存和扩展的高价值客户。
  不要用于生成用户画像（用user-personas）、市场细分（用market-segments）或客户旅程分析（用customer-journey-map）。
  触发词：理想客户画像、ICP、客户画像、目标客户定义、PMF分析
