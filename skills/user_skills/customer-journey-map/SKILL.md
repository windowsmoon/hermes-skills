...
description: >
  当需要绘制端到端客户旅程地图（从认知到推荐）时使用。
  用于识别触点、情绪曲线、痛点、增长机会，改进用户引导流程和用户体验。
  不要用于用户画像分析（用user-personas）、A/B测试分析（用ab-test-analysis）或非客户体验类的流程分析。
  触发词：客户旅程、用户旅程、用户体验地图、旅程地图、用户路径
