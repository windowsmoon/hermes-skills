...
name: job-stories
description: >
  当需要编写Job Stories（JTBD格式的用户故事）时使用。
  用于按"当[情境]时，我想[动机]，以便[结果]"格式创建带验收标准的用户故事。
  不要用于传统用户故事编写（用user-stories）、PRD编写（用create-prd）或功能点列表。
  触发词：Job Story、JTBD、用户故事、情境驱动、任务故事
