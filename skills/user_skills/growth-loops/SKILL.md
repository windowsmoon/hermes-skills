...
name: growth-loops
description: >
  当需要为产品设计可持续的增长飞轮（增长回路）时使用。
  用于评估5种增长回路（病毒式/使用量/协作/UGC/推荐），设计产品驱动增长（PLG）机制。
  不要用于增长指标仪表盘设计（用metrics-dashboard）、单次营销活动策划（用marketing-ideas）或付费广告策略。
  触发词：增长回路、增长飞轮、增长循环、病毒增长、PLG
