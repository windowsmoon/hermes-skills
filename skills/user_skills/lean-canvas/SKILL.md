...
name: lean-canvas
description: >
  当需要快速生成精益画布（Lean Canvas）进行商业假设测试时使用。
  用于覆盖问题/解决方案/指标/成本结构/独特价值主张/不公平优势/渠道/细分/收入9大模块。
  不要用于完整的产品战略（用product-strategy或startup-canvas）、已成熟的商业模式记录（用business-model）或非创业场景的通用分析。
  触发词：精益画布、Lean Canvas、创业画布、商业模式假设、快速创业模型
