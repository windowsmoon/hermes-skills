...
description: >
  当用户需要创建富文本Markdown文档（含图表/流程图/思维导图）时使用。
  支持：Mermaid图表、数学公式、数据可视化、技术架构图。
  不要用于：PPT生成（走smart-ppt）、HTML页面（走frontend-design）、纯文本笔记。
  触发词：Markdown文档、技术文档、数据可视化、Mermaid图表、流程图
version: 1.0.0
author: Ekko
license: MIT
...
> ⚠️ **Kroki 已被评估为不可用** — 公共实例 404，无 CLI 工具，Docker 自部署门槛高。**替代方案：mmdc (Mermaid CLI) 已全局预装**，可直接用于从 `.mmd` 文件生成思维导图/流程图等 PNG/SVG/PDF 图片。参见 `references/kroki.md` 获取详细评估、mmdc 用法和对比。
